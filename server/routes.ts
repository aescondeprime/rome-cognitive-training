...
import { insertTrialSchema, insertSessionSchema, insertRecallItemSchema } from "@shared/schema";
import { z } from "zod";
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/user", (req, res) => {
    const user = storage.getDefaultUser();
    res.json(user);
  });
  ...
  app.get("/api/trials/recent", (req, res) => {
    const user = storage.getDefaultUser();
    const trials = storage.getRecentTrials(user.id, 100);
    res.json(trials);
  });
  ...
  app.patch("/api/recall-items/:id/review", (req, res) => {
    ...
    const items = storage.getRecallItems(storage.getDefaultUser().id);
